...
معادله بالا می‌تواند به شکل یک معادله چندجمله‌ای تکین (monic polynomial) به فرم زیر تبدیل شود و از طریق روش [Coppersmith](https://en.wikipedia.org/wiki/Coppersmith%27s_attack)  حل شود. در این رمزنگاری RSA مقدار $e=3$ و مقدار pad رندم اضافه شده متن کوچک باشد با  $f$ نسبت به $n$ به اندازه کافی کوچک باشد، می‌توانیم ریشه صحیح $F$ را از معادله پیدا کنیم. 
...
در اینجا برای اینکه معادله را به یک معادله تکین تبدیل کنیم یعنی ضریب $x^3$ یک شود، باید ضرایب را بر $K$ تقسیم کنیم: 
